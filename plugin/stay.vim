...
if exists("b:loaded_stay_plugin") || &cp
	finish
endif
let b:loaded_stay_plugin = 1

scriptencoding utf-8

if !exists('g:stay_ignore')
	let g:stay_ignore = "gitcommit,gitrebase,svn,hgcommit"
endif

if !exists('g:stay_open_folds')
	let g:stay_open_folds = 1
endif

if !exists('g:stay_ignore_buftype')
	let g:stay_ignore_buftype = "quickfix,nofile,help"
endif

fu! s:stay()
	if index(split(g:stay_ignore_buftype, ","), &buftype) != -1 
		return
   	endif

	if index(split(g:stay_ignore, ","), &filetype) != -1
		return
	endif
	...
	if foldclosed(".") != -1 && g:stay_open_folds
